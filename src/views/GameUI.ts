namespace ui
{
    export class GameUI extends core.BaseNode
    {
        ...
        private countdown: core.Countdown;
        ...
        constructor(duration: number)
        {
            ...
            this.countdown = new core.Countdown(duration * 1000);
        }
        ...
        public set lastTappedCell(value: Cell)
        {
            ...
            if (value)
                core.SoundUtils.play('click1_mp3');
        ...
        public start()
        {
            ...
            this.asyncCountdown().then(v => this.stop()).catch(error =>
            {
            }); //直到倒计时结束
            this.renderCheat();
        ...
        public async renderDead(): Promise<any>
        {
            ...
            this.removeChild(deadSprite); //移除死局遮罩
            this.resume();
        }
        ...
        public async swapAndCrush(fromCell: Cell, toCell: Cell, crushedCells: CrushedCells)
        {
            ...
            while (crushedCells.hasCrushes && this.running)
            {
                if (serials > 0)
                {
                    this.playSound(('syllable_' + (serials % 9) + '_mp3'));
                } else
                {
                    this.playSound('normal_mp3');
                }
                serials++;
        ...
        {
            if (event.cell.block || !this.isActive) return;
        ...
        private onCellTap(event: CellEvent)
        {
            ...
            {
                this.lastTappedCell = event.cell;
            } else if (this.lastTappedCell instanceof Cell)
            { // 有上一次的点击, 需要判断是否需要swap
                if (
