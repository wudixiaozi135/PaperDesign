namespace ui
{
    export class CellUI extends core.BaseNode
    {
        ...
        private render(): void
        {
            ...
            if (!isNaN(this.cell.color))
            { //色块
                rect.graphics.beginFill(this.cell.color);
                rect.graphics.drawRoundRect(0, 0, this.width, this.height, 5);
                rect.graphics.endFill();
            } else if (typeof this.cell.color == 'string')
            { // 位图名称
                let bitmap: core.BitmapUI = new core.BitmapUI(this.cell.color);
                bitmap.x = 8;
                bitmap.y = 8;
                bitmap.width = this.width - 16;
                bitmap.height = this.height - 16;
                this.addChild(bitmap);
            } else if (this.cell.color instanceof Function)
            { // 回调函数，自行制作sprite
